...
```{r}
gates <- read.delim("../data/gates_foundation/raw/gates_gh_17.tsv",
                    stringsAsFactors = FALSE)
```

### Data Processing 

* Remove "$" and commas from amount field  
* Add Gates Foundation as funder  
* Add "Private sector" as funder_type  
* Add "Gates Foundation" as data source

```{r}
## remove commas and $ from amount
gates$AMOUNT <- gsub(",", "", gsub("\\$", "", gates$AMOUNT))

## add funder == "Gates Foundation"
gates$funder <- "Gates Foundation"

## funder_type == "Private sector"
gates$funder_type <- "Private sector"

gates$data_source <- "Gates Foundation"
```
...
if(csv_flag == TRUE){
  write.csv(gates,
            file = "../data/gates_foundation/processed/gates_processed.csv", 
            row.names = FALSE)
}
```
...
## generate dataset that includes the core data elements included in gates,
## as specified in the field_mapping guide printed above
gates_core_data <- gates[which(names(gates) %in% field_mapping$fieldname[
                                which(field_mapping$source == "Gates" &
                                      complete.cases(field_mapping$standard_fieldname))])]

## rename core data elements of gates

## first create temporary dataframe that merges names of core data to standard_fieldnames specified in field_mapping guide
temp_names <- suppressWarnings(inner_join(data.frame(fieldname = names(gates_core_data)), 
                   field_mapping[which(field_mapping$source == "Gates"),],
                   by = "fieldname"))

## then update gavi_core_data fieldnames
names(gates_core_data) <- temp_names$standard_fieldname; rm(temp_names)
```
...
```{r}
## UNION ALL (rbind) data together for Gates, OIE, IHME, and G-Finder, plus match fieldnames
core_data <- plyr::rbind.fill(gates_core_data,
                              gavi_core_data,
                              gfinder_core_data,
                              ihme_core_data,
                              oie_core_data)
...
```{r}
application_area_mapping <- field_mapping <- read.table("../data/application_area_mapping.tsv", 
                                                        stringsAsFactors = FALSE, 
                                                        sep = "\t", header = TRUE)
```

```{r}
for(i in 1:nrow(application_area_mapping)){
  
  ## column index of the field that needs to be recoded
  colindex <- which(names(core_data) == application_area_mapping$initial_field[i])
  
  ## look at rows that correspond to the specified value, save those rows
  rowindex <- which(core_data[,colindex] == application_area_mapping$initial_value[i])
  
  ## column index of the field that will be replaced
  colindex_update <- which(names(core_data) == application_area_mapping$new_field[i])
  ...
  core_data[rowindex,colindex_update] <- application_area_mapping$new_value[i]
...
```{r}
jee_mapping <- field_mapping <- read.table("../data/jee_mapping.txt", 
                                          stringsAsFactors = FALSE, 
                                          sep = "\t", header = TRUE)
```

```{r}
for(i in 1:nrow(jee_mapping)){
  
  ## column index of the field that needs to be recoded
  colindex <- which(names(core_data) == jee_mapping$initial_field[i])
  
  ## look at rows that correspond to the specified value, save those rows
  rowindex <- which(core_data[,colindex] == jee_mapping$initial_value[i])
  ...
  core_data[rowindex,which(names(core_data) == "core_element")] <- jee_mapping$core_element[i]
  core_data[rowindex,which(names(core_data) == "core_capability")] <- jee_mapping$core_capability[i]
...
The sampled dataset used as part of the demo dataset will include data from the following sources:  
  ...
  * G-Finder (random sample of 100 committed funds from 2015 only)
  * Gates Foundation (random sample of 100 committed funds from 2017 only)
...
set.seed(1202)
## store the indices of the gfinder data elements we want to include, in case we want to reference them later
...
set.seed(1202)
## store the indices of the Gates data elements we want to include, in case we want to reference them later
gates_sample_indices <- sample(which(core_data$data_source == "Gates Foundation"), size = 100)

## demo data combining selected data elements, identified in the notes above
demo_data_actual <- plyr::rbind.fill(core_data[gavi_sample_indices,],
                                     core_data[gfinder_sample_indices,],
                                     core_data[oie_sample_indices,],
                                     core_data[gates_sample_indices,])
...
```{r, warning = FALSE}
ggplot(demo_data, aes(log(amount_committed), fill = data_source)) +
    geom_histogram(bins = 25) +
  ylab("Number of Commitments") +
  xlab("Log Amount committed") +
  ggtitle("Commitment Amount by Data Source") +
  ## rename legend
  scale_fill_discrete(name = "Data Source")
```
...
This should have a legend, but not going to spend the time doing it in R since it's a decent amount of work for this type of plot (boo ggplot2 for not allowing more manual overrides).  
