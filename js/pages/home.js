...
(() => {
	...
	*/
	...
	function buildMap(selector, params = {}) {
		...
		const legendBottomPos = {opening: '-3px', shutting: '-81px'};
		$('.legend-display-tab').click(function toggleLegendDisplay() {
			const $arrow = $(this).find('.collapse-arrow').toggleClass('rotated');
			const shutting = $arrow.hasClass('rotated');
			$(this).find('span').text(shutting ? 'show legend' : 'hide legend');
			$('.legend-content').slideToggle();
			$('.legend-position').animate({bottom: shutting ? legendBottomPos.shutting : legendBottomPos.opening });
		});
